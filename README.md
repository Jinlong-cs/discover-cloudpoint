...
```bash
git clone https://github.com/<your-org>/discover-pointcloud.git
cd discover-pointcloud
...
加载 GT 和 Prediction 后，点击任意一侧点云中的点：

- 软件会按同像素或最近投影像素在另一侧查找对应点。
- 两侧点云会同时高亮该点。
- 左侧“像素对比”面板会显示 `GT xyz`、`Pred xyz`、`Pred - GT xyz`、3D 距离、视差差值。
...
如果要直接调试根目录前端，可以在上一级 `Discover` 目录启动静态服务：
